...
//                    modelNames: ['PortfolioItem/' + ptype.rawValue], //NOOOO!
    ...
    _copyTags: function(){
        ...
        if ( gApp._getSourceWorkspace().ObjectID !== gApp._getTargetWorkspace().ObjectID ) {
            ...
            _.each(tags, function(tag) {
                ...
                var url = gApp.getContext().getSubscription()._ref;
                url = url.substring(0,url.indexOf('subscription')) +
                    'tag/create';
                getReq.onreadystatechange = gApp._tagSuccessHandler;
                getReq.withCredentials = true;
                currentState = 'Reading';
                getReq.open("POST", url + '?compact=true&includePermissions=true&project=' + encodeURIComponent(gApp._getTargetProject()._ref));
                getReq.send(JSON.stringify(msg));
    ...
    _tagSuccessHandler: function(event) {
