...
//                    modelNames: ['PortfolioItem/' + ptype.rawValue], //NOOOO!
    ...
    _copyTags: function(){
        ...
        if (( gApp._getSourceWorkspace().ObjectID !== gApp._getTargetWorkspace().ObjectID ) && (tags.length > 0)){
            var tagStore = Ext.create('Rally.data.wsapi.Store', {
                ...
                listeners: {
                    load: function(store, records) {
                        ...
                        //If no tags added, then store.sync does nothing
                        store.sync().then ({
                            success: function() {
            ...
            //We don't need to do anything as the tags are already there.
            deferred.resolve('copyTags');
        }
        return deferred.promise;
    },
    ...
    _attachRevisionHistory: function(){
        ...
        gApp.setLoading('Copying revision histories as notes');

        gApp._nodeTree.each( function(node) {
            var nodeDeferred = Ext.create('Deft.Deferred');
            ...
            sourceStore.load({
                callback: function(records, operation, success) {
                    if (success) {
                        var revisionText = '<div>Revision History of Original<p/></div><table style="width:100%"><tr><th>TimeStamp</th><th>By</th><th>Detail</th></tr>';
                        _.each(records, function(record) {
                            var shortDate = Ext.Date.format(record.get('CreationDate'),'d-M-Y H:i:s');
                            revisionText += '<tr><td>' + gApp._fixedStringLength(shortDate, 25) + ' ' +
                                '</td><td>' + gApp._fixedStringLength(record.get('User')._refObjectName, 25) + ' ' +
                                '</td><td>' + record.get('Description') +
                                '</td></tr>';
                        });
                        revisionText += '</table>';
                        var notes = node.data.targetRecord.get('Notes');
                        node.data.targetRecord.set('Notes', notes + '\n' + revisionText);
                        node.data.targetRecord.save({
                            callback: function() {
        ...
        Deft.Promise.all(promises).then({
            success: function() {
                deferred.resolve('attachRevisionHistory');
            },
            failure: function() {
                deferred.reject('attachRevisionHistory');
            }
        }). always ( function() {
                gApp.setLoading(false);
        });
    ...
    _saveRecord: function(node) {
        ...
        var fieldConfig = {};
        var data = {};
        var copyList = gApp._copyFieldList(sourceType);
        _.each( copyList, function(fieldName) {
        ...
        fieldConfig.Notes = ((data.Notes !== undefined)?data.Notes:'') +
                        '<div>Created from ' + node.data.sourceRecord.get('FormattedID') + 
                        ' (from workspace: ' + gApp._getSourceWorkspace().Name + ' ) </div>' ;
        ...
        node.data.targetRecord.save().then({
            success: function(savedRecord) {
                ...
                // //Reload the parent for updated fields
                // if ( node.parent !== null) { 
